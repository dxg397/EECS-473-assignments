...
int main(int argc, char** argv) {
    ...
    //getting the faulty part away from inspector one
   ...
   for (int i=0;i<nparts;i++) {
    ...
    //when the inspector find the bad part
    //repeat no bad part found
    while (boxInspector.get_bad_part_Q1(current_part)) {
        ...
        cin>>ans;
        if(ans==1){
          //find the bad part in the logical box_camera_1
          float delta=-1;
          int bad_part_index=-1;
          nparts = orphan_models_wrt_world.size();
          // ROS_INFO("num parts seen in box = %d",nparts);
          //find the closet part in the logic camera;
          for (int i=0;i<nparts;i++) {
             int logic_x=orphan_models_wrt_world[i].pose.position.x;
             int logic_y=orphan_models_wrt_world[i].pose.position.y;
             int quality_x=current_part.pose.pose.position.x;
             int quality_y=current_part.pose.pose.position.y;
             int delta_temp=(logic_x-quality_x)*(logic_x-quality_x)+(logic_y-quality_y)*(logic_y-quality_y);
             if(delta==-1)
             {
               delta=delta_temp;
               bad_part_index=i;
          ...
          //void model_to_part(osrf_gear::Model model, inventory_msgs::Part &part, unsigned short int location)
          model_to_part(orphan_models_wrt_world[bad_part_index], current_part, inventory_msgs::Part::QUALITY_SENSOR_1);
          ROS_INFO("Attempting to remove bad part: ");
          ROS_INFO_STREAM(current_part<<endl);
          ...
          //ALSO, WATCH OUT FOR NO PARTS IN THE BOX--ABOVE WILL CRASH
              ...
              ROS_INFO("Re-inspecting for the bad part ");
              boxInspector.update_inspection(desired_models_wrt_world,
                  satisfied_models_wrt_world,misplaced_models_actual_coords_wrt_world,
                  misplaced_models_desired_coords_wrt_world,missing_models_wrt_world,
                  orphan_models_wrt_world,part_indices_missing,part_indices_misplaced,
          ...
          ROS_INFO("Will not remove the bad part, going to Q2");
        }
    }
    ROS_INFO("no bad part found, moving to Q2");
    ...
    // ROS_INFO("num missing parts in box = %d", missing_models_wrt_world.size());
    for (int i = 0; i < part_indices_missing.size(); i++) {
        ROS_INFO_STREAM("part_indices_missing: " << part_indices_missing[i]<< endl);
    }
    for (int i = 0; i < nparts; i++) {
        ROS_INFO_STREAM("orphaned  parts: " << orphan_models_wrt_world[i] << endl);
    }
    // for (int i = 0; i < desired_models_wrt_world.size(); i++) {
    //     ROS_INFO_STREAM("desired_models_wrt_world  parts: " << desired_models_wrt_world[i]<< endl);
    // }
    // for (int i = 0; i < desired_models_wrt_world.size(); i++) {
    //     ROS_INFO_STREAM("misplaced_models_desired_coords_wrt_world  parts: " << misplaced_models_desired_coords_wrt_world[i]<< endl);
    // }
    // for (int i = 0; i < desired_models_wrt_world.size(); i++) {
    //     ROS_INFO_STREAM("missing_models_wrt_world  parts: " << missing_models_wrt_world[i]<< endl);
    // }
    ...
    bool go_on = false;
     ...
     // should repeat for ALL  missing parts
         ...
         int nparts_missing = missing_models_wrt_world.size();
         ROS_INFO("found %d missing parts", nparts_missing);
         // if(nparts_missing>0){
