...
int main(int argc, char** argv) {
    ...
    // precision value
    // value before which the robot should stop
    float offset = 0.04,xoffset,yoffset;
    //cin>>offset;
    // radius of the gear part
    float radius = 0.07,xradius,yradius;
    // targated x coordinates 
    float x_cord = 0.0;
    // targated y coordinates
    float y_cord = 0.0 ;  
    cout<<"Enther the x-coordinates:- ";
    cin>>x_cord;
    ...
    // if the value of the targated x coordinate is greater than the current value than the 
    // robot must be an radius behind the current position of the gear part and must stop before an offset
    // else it must be forward 
    if( g_perceived_object_pose.pose.position.x < x_cord)
    {
    xoffset = offset*-1;
    xradius = radius*-1;
    }
    float new_xpos = g_perceived_object_pose.pose.position.x + radius;
    float new_targ_x_pos = x_cord + offset;
    //go to pose to touch top of gear part; same x and y, but lower z value for flange origin
    flange_origin <<new_xpos, g_perceived_object_pose.pose.position.y, 0.005; 
    ...
    // move horizontally in x-axis direction
    flange_origin << new_targ_x_pos , g_perceived_object_pose.pose.position.y,0.005;
    ...
    print_traj(new_trajectory);
    traj_publisher.publish(new_trajectory); //publish the trajectory
    ros::Duration(3.0).sleep(); //wait for the motion
 ...
 //###################################################################################
    ...
    // compute the object position again and now change the values wrt to y coordinates
    // move horizontally in y-axis direction
    ...
    // robot must be an radius behind the current position of the gear part and must stop before an offset
    // else it must be forward 
    if( g_perceived_object_pose.pose.position.y < y_cord)
    {
    yoffset = offset*-1;
    yradius = radius*-1;
    }

    float new_ypos = g_perceived_object_pose.pose.position.y + yradius;
    float new_targ_y_pos = y_cord + yoffset;
    //go to pose to touch top of gear part; same x and y, but lower z value for flange origin
    flange_origin <<g_perceived_object_pose.pose.position.x, new_ypos, 0.005; 
    ...
    // move horizontally in x-axis direction
    flange_origin <<   g_perceived_object_pose.pose.position.x, new_targ_y_pos ,0.005;
    ...
    ros::Duration(3.0).sleep(); //wait for the motion
     ...
     //depart vertically: same x and y, but move to higher z value
    //flange_origin << g_perceived_object_pose.pose.position.x, g_perceived_object_pose.pose.position.y, 0.5;
    flange_origin <<   g_perceived_object_pose.pose.position.x, new_targ_y_pos ,0.5;
    goal_flange_affine.translation() = flange_origin;
    ROS_INFO_STREAM("move to flange origin= " << goal_flange_affine.translation().transpose() << endl);
    g_q_vec_arm_Xd = optimal_path.back(); //start from the joint-space pose that ended the prior plan
    //convert move to an optimal joint-space path:
    ...
    traj_publisher.publish(new_trajectory); //publish the trajectory
    ros::Duration(arrival_time).sleep(); //wait for the motion
    ROS_INFO("Completed the trajectory :>");
